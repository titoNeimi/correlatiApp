...
export default function CareerDetailPage() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      setError(null)
      try {
        const [programData, subjectsData] = await Promise.all([
          fetchProgram(id),
  ...
  const subjectsByYear = useMemo(() => {
    return subjects.reduce<Record<number, DegreeProgramSubject[]>>((acc, subject) => {
            ...
            {years.length === 0 ? (
              ...
              <div className="grid gap-6 lg:grid-cols-[220px_1fr]">
                <div className="bg-slate-50 rounded-2xl border border-slate-100 p-4 h-fit">
                  <p className="text-xs uppercase tracking-wide text-slate-500 mb-3">Años</p>
                ...
                <div className="space-y-4">
                  <div className="flex items-center justify-between">
                    <h3 className="text-lg font-semibold text-slate-900">
                      {selectedYear ? `${selectedYear}° Anio` : 'Selecciona un anio'}
                    </h3>
